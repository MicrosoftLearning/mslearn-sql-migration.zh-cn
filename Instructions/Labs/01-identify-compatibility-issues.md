...
1. 在“步骤 2: **** 评估结果和 SKU 建议”中，等待评估完成并选择“下一步”****。

## 查看评估结果

现在，您可以查看迁移扩展生成的建议。

1. 在“步骤 3: **** 目标平台和评估结果”中，选择“Azure SQL 数据库”作为目标平台****。

1. 选择*AdventureWorks*数据库。 花点时间查看右侧的评估结果。
    
    > **注意：** 我们可以看到，之前添加的`Next`列已被标记，因为它可能会导致 Azure SQL 数据库出错。

1. 改为选择“Azure SQL 托管实例”作为“Azure SQL 数据库”目标平台********。
...
1. 返回到向导中的“步骤 2: **** 评估结果和 SKU 建议”页，然后选择“刷新评估”****。

1. 选择“Azure SQL 数据库”作为目标平台****。
